...
binning_time = 0.01
t_total = positions[0][-1]*1000 # must be in ms
speed = 20.0
delta_t = binning_time*1000 # position sampling rate in ms
...
for rng_seed in np.arange(int(rand_seed), int(rand_seed)+50, 1):
    ...
    rat_trajectory = randomwalk.running_rat(t_total+delta_t, timestep=delta_t, velocity=speed, x_lim=74.5, y_lim=74.5)

    x_t = rat_trajectory[:,0]; y_t = rat_trajectory[:,1] # Interpolate the midpoint positions of the LEDs
    results['x_t'] = x_t
    results['y_t'] = y_t
    ...
    io.savemat('GridCell_Module%d_randomwalk_seed%05d.mat'%(int(lambda_0), int(rng_seed)), results)
