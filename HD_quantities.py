...
def HD_tuningcurves(N_neurons, spike_trains, head_direction, binning_time, N_bins=40):
    ...
    if N_neurons > 1:
        ...
        try: # do this when the positions are masked arrays
            numer = np.histogram(np.repeat(np.ravel(head_direction),np.ravel(spike_trains).astype("int")).compressed(), bins=N_bins, range=(0,2.0*np.pi))[0]
        except:
            numer = np.histogram(np.repeat(np.ravel(head_direction),np.ravel(spike_trains).astype("int")), bins=N_bins, range=(0,2.0*np.pi))[0]
...
def HD_sparsity(N_neurons, spike_trains, head_direction, binning_time, N_bins=40, output_name=None):
    ...
    if N_neurons > 1:
        ...
        spike_map = np.ma.array(ratemap, mask=np.isnan(ratemap))
        angular_sparsity = 1. - np.power(np.ma.sum(denom*spike_map)/np.ma.sum(denom),2)/(np.ma.sum(denom*np.power(spike_map,2))/np.ma.sum(denom))
